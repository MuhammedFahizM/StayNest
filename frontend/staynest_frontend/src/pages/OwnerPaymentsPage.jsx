...
export default function OwnerPaymentsPage() {
  ...
  const registerMonthRef = useRef(registerMonth);
  const activeTabRef = useRef("tenants");
  const tenantSubTabRef = useRef("all");
  const monthLedgerLoadedRef = useRef({ loaded: false, month: selectedMonth });
  ...
  useEffect(() => { registerMonthRef.current = registerMonth; }, [registerMonth]);
  useEffect(() => { activeTabRef.current = activeTab; }, [activeTab]);
  useEffect(() => { tenantSubTabRef.current = tenantSubTab; }, [tenantSubTab]);
  ...
  useEffect(() => {
    const silentPoll = async () => {
      ...
      if (tab === "tenants") {
        fetchTenantList(propId);
        fetchUpcoming(propId);
        if (bookingId) fetchTenantDetail(bookingId);
        if (tenantSubTabRef.current === "month" && monthLedgerLoadedRef.current.loaded) {
          fetchMonthLedger(propId, monthLedgerLoadedRef.current.month);
        }
      }
      if (tab === "deposits") fetchDeposits(propId);
      if (tab === "offline") fetchRegister(propId, registerMonthRef.current);
    };
  ...
  const handleMarkOffline = async (entry, type) => {
    ...
    // Optimistic: flip owner-confirmed flag immediately
    const patch = type === "RENT"
      ? { rent_offline_confirmed_by_owner: true }
      : { food_offline_confirmed_by_owner: true };

    // Update tenant detail ledger
    setTenantDetail(prev => prev ? { ...prev, ledger: prev.ledger.map(e => e.id === entry.id ? { ...e, ...patch } : e) } : prev);
    // Update month ledger
    setMonthLedger(prev => prev.map(e => e.id === entry.id ? { ...e, ...patch } : e));

    try {
      await markOfflinePayment(entry.id, type, type === "RENT" ? entry.rent_amount : entry.food_amount);
      ...
      // Revert
      const revert = type === "RENT"
        ? { rent_offline_confirmed_by_owner: false }
        : { food_offline_confirmed_by_owner: false };
      setTenantDetail(prev => prev ? { ...prev, ledger: prev.ledger.map(e => e.id === entry.id ? { ...e, ...revert } : e) } : prev);
      setMonthLedger(prev => prev.map(e => e.id === entry.id ? { ...e, ...revert } : e));
  ...
  const handleConfirmOffline = async (entry, type) => {
    ...
    const patch = type === "RENT"
      ? {
        rent_status: "PAID_OFFLINE", rent_offline_confirmed_by_owner: true, rent_paid_at: now,
        overall_status: (entry.food_status === "NA" || entry.food_status === "PAID_ONLINE" || entry.food_status === "PAID_OFFLINE") ? "PAID" : "PARTIAL",
      }
      : {
        food_status: "PAID_OFFLINE", food_offline_confirmed_by_owner: true, food_paid_at: now,
    ...
    try {
      ...
      // Revert
      const revert = type === "RENT"
        ? { rent_status: entry.rent_status, rent_offline_confirmed_by_owner: false, rent_paid_at: entry.rent_paid_at, overall_status: entry.overall_status }
        : { food_status: entry.food_status, food_offline_confirmed_by_owner: false, food_paid_at: entry.food_paid_at, overall_status: entry.overall_status };
      setTenantDetail(prev => prev ? { ...prev, ledger: prev.ledger.map(e => e.id === entry.id ? { ...e, ...revert } : e) } : prev);
      setMonthLedger(prev => prev.map(e => e.id === entry.id ? { ...e, ...revert } : e));
      toast.error(err?.response?.data?.error || "Failed");
    } finally { setOfflineLoading(null); }
  ...
  const handleFoodResponse = async (reqId, action) => {
    ...
    setFoodRequests(prev => prev.filter(r => r.id !== reqId));
              ...
              {tenantSubTab === "month" && (
                <>
                  <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 16, flexWrap: "wrap" }}>
                    ...
                    />
                    <button onClick={() => { monthLedgerLoadedRef.current = { loaded: true, month: selectedMonth }; fetchMonthLedger(selectedProperty, selectedMonth); }}
